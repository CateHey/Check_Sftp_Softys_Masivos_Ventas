...
def SFTPLagui():
    ...
    with pysftp.Connection(host=myHostname, username=myUsername, password=myPassword,cnopts=cnopts) as sftp:
            ...
            sftp.cwd('/home/lagui_protisa/Data/')        
            directory_structure = sftp.listdir_attr()
            remoteFile = '/home/lagui_protisa/Data/ventas.csv'
                            ...
                            if dia==0:
                                try:
                                    ...
                                    if ((date.today() - dateutil.relativedelta.relativedelta(months=2)) < fechak ) and (fechak < (date.today() -  dateutil.relativedelta.relativedelta(days=2))):                                                
                                        a = "SOFTYS","LAGUI",attr.filename,attr.st_size, time.strftime("%d-%m-%Y" ,time.localtime(attr.st_mtime)),time.strftime("%H:%M" ,time.localtime(attr.st_mtime)),k                                                
                                        info.append(a)
                                except:
